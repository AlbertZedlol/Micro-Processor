...
############################################################################################################################################################
.data   0x00000040                      # start of the data address(buffer). the 16-th element in RAM.
.align 4                                # filename: .asciiz "test.dat"
buffer:   .space  260   #4100           shrink buffer to a 8x8 matrix, with an additional first element [0] indicating number of nodes(n=6)
dist:     .space  80    #128
visited:  .space  80    #128
    ...
    for_2:
        ...
        update:
            ...
            for_4:
                ...
                sll $a1, $t8, 3             #addr=(u<<3)                                        ##!!!!!!!!!!!!!shrink the graph to 64.!!!!!!!!!
                add $a1,$a1, $t9            #addr=(u<<3)+v (in Words)
                sll $a1, $a1, 2             #addr=(u<<3)+v (in Bytes)
